document.addEventListener('DOMContentLoaded', function() {
    ...
    // Update Play/Pause Button Text and Icon Based on Video State
    function updatePlayPauseButton() {
        if (video.paused || video.ended) {
            playPauseBtn.innerHTML = '<span class="video-control-svg"><svg xmlns="http://www.w3.org/2000/svg" width="19" height="20" fill="none" viewBox="0 0 19 20">\n' +
            ...
            playPauseBtn.innerHTML = '<span class="video-control-svg"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">\n' +
    ...
    // Update Mute/Unmute Button Text and Icon Based on Mute State
    function updateMuteUnmuteButton() {
        if (video.muted) {
            muteUnmuteBtn.innerHTML = '<span class="video-control-svg"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">\n' +
                '  <path stroke="#fff" stroke-width="1.5" d="M15 6.37c0-1.712-2.012-2.632-3.307-1.513l-3.33 2.876a2 2 0 0 1-1.307.487H5.5a2.5 2.5 0 0 0-2.5 2.5v3.293a2.5 2.5 0 0 0 2.5 2.5h1.556a2 2 0 0 1 1.307.486l3.33 2.876c1.295 1.12 3.307.2 3.307-1.513V6.37Z"/>\n' +
                '  <path stroke="#fff" stroke-linecap="round" stroke-width="1.5" d="M18 15.367c.628-.836 1-1.875 1-3a4.977 4.977 0 0 0-1-3m2 9a9.954 9.954 0 0 0 2-6 9.954 9.954 0 0 0-2-6"/>\n' +
                '</svg>\n' +
                '</span> Unmute';
        } else {
            muteUnmuteBtn.innerHTML = '<span class="video-control-svg"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">\n' +
    ...
    playPauseBtn.addEventListener('click', function() {
        ...
        updatePlayPauseButton(); // Immediately update the button
    });

    // Toggle Mute/Unmute on Button Click
    muteUnmuteBtn.addEventListener('click', function() {
        video.muted = !video.muted;
        updateMuteUnmuteButton(); // Immediately update the button
    });

    // Restart the Video on Button Click
    restartBtn.addEventListener('click', function() {
        video.currentTime = 0; // Reset to the beginning
        video.pause(); // Pause the video first to ensure it stops completely
        video.play().then(() => {
            // Successfully playing the video
            updatePlayPauseButton(); // Set button to "Pause" after restarting
        }).catch((error) => {
    ...
    video.addEventListener('volumechange', updateMuteUnmuteButton);
